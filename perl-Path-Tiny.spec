%{?scl:%scl_package perl-Path-Tiny}
...
Name:		%{?scl_prefix}perl-Path-Tiny
Version:	0.108
Release:	5%{?dist}
...
BuildRequires:	%{?scl_prefix}perl(Digest) >= 1.03
BuildRequires:	%{?scl_prefix}perl(Digest::MD5)
BuildRequires:	%{?scl_prefix}perl(Digest::SHA) >= 5.45
...
BuildRequires:	%{?scl_prefix}perl(File::Path) >= 2.07
BuildRequires:	%{?scl_prefix}perl(File::Spec) >= 0.86
BuildRequires:	%{?scl_prefix}perl(File::stat)
...
BuildRequires:	%{?scl_prefix}perl(Test::MockRandom)
%endif
# Runtime
Requires:	%{?scl_prefix}perl(:MODULE_COMPAT_%(%{?scl:scl enable %{scl} '}eval "$(perl -V:version)";echo $version%{?scl:'}))
Requires:	%{?scl_prefix}perl(Cwd)
Requires:	%{?scl_prefix}perl(Digest) >= 1.03
Requires:	%{?scl_prefix}perl(Digest::MD5)
Requires:	%{?scl_prefix}perl(Digest::SHA) >= 5.45
...
Requires:	%{?scl_prefix}perl(File::Path) >= 2.07
Requires:	%{?scl_prefix}perl(File::stat)
Requires:	%{?scl_prefix}perl(File::Temp) >= 0.18
Requires:	%{?scl_prefix}perl(threads)
Requires:	%{?scl_prefix}perl(warnings::register)

# For performance and consistency
%if !(0%{?rhel}) && ! (0%{?scl:1})
BuildRequires:	%{?scl_prefix}perl(PerlIO::utf8_strict) >= 0.003
Requires:	%{?scl_prefix}perl(PerlIO::utf8_strict) >= 0.003
%endif
BuildRequires:	%{?scl_prefix}perl(Unicode::UTF8) >= 0.58
Requires:	%{?scl_prefix}perl(Unicode::UTF8) >= 0.58
...
%changelog
* Fri Jan 03 2020 Jitka Plesnikova <jplesnik@redhat.com> - 0.108-5
- SCL
